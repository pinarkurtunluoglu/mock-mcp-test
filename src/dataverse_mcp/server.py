...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "## Turkish → Column Mapping\n"
        "When the user says:\n"
        "- 'ürün' / 'malzeme' / 'isim' / specific product name (e.g. 'Organik Mısır') → use `mserp_itemname`\n"
        "- 'ana grup' / 'ürün grubu' / 'seviye 2' / Level-2 category names (e.g. 'Tahıl', 'Bakliyat', 'Yağlı Tohum') → use `mserp_etgproductlevel02name`\n"
        "- 'alt kategori' / 'kategori' / 'seviye 3' / Level-3 category names (e.g. 'Buğday', 'Mısır', 'Soya Küspesi', 'Nohut') → use `mserp_etgproductlevel03name`\n"
        "- **CRITICAL — Category vs Product disambiguation**: Commodity names like 'Buğday', 'Mısır', 'Soya Küspesi', 'Nohut', 'Ayçiçeği' exist BOTH as Level-3 category values AND inside individual product names. Choose the field based on intent:\n"
        "  • **Aggregation intent** (toplam miktar/değer/yaş, group breakdown) → use `mserp_etgproductlevel03name`. Example: 'Soya Küspesi toplam miktarı' → filter `contains(mserp_etgproductlevel03name, 'Soya Küspesi')`\n"
        "  • **Specific product/SKU lookup** (belirli ürün, lot, stok kalemi arama) → use `mserp_itemname`. Example: 'Soya Küspesi lot ara' → filter `contains(mserp_itemname, 'Soya Küspesi')`\n"
        "  • **Default rule**: when in doubt about aggregation vs lookup, prefer `mserp_etgproductlevel03name` for commodity names.\n"
        "- **Category level unknown**: If the user mentions a category name and you are NOT sure whether it is Level 2 or Level 3, "
