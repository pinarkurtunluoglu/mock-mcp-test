...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "- **CRITICAL — Category vs Product disambiguation**: Commodity names like 'Buğday', 'Mısır', 'Soya Küspesi', 'Nohut', 'Ayçiçeği' exist BOTH as Level-3 category values AND inside individual product names. These are TWO DIFFERENT searches:\n"
        "  • **Seviye 3 / kategori** → `contains(mserp_etgproductlevel03name, 'Soya Küspesi')` — tüm Soya Küspesi stoğu (38.6M kg)\n"
        "  • **Ürün adı / spesifik** → `contains(mserp_itemname, 'Soya Küspesi')` — sadece bu isimli SKU'lar (çok daha az)\n"
        "  • If ambiguous, ASK: 'Seviye 3 kategorisi olarak mı (tüm kategori stoğu), yoksa spesifik ürün adına göre mi arama yapayım?'\n"
        "  • After user clarifies, use EXACTLY the field they indicated — NEVER mix them up.\n"
